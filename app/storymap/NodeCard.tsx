...
const NodeCard: React.FC<NodeItems> = ({ isDark, title, author, createdAt, currId }) => {
    return (
        <div className={`w-72 p-4 rounded-xl ${isDark ? 'node-card-dark' : 'node-card-light'} shadow-md relative overflow-visible border-border transition-all duration-300 hover:shadow-xl hover:scale-105`}>
            <div className="absolute -top-2 -right-2 flex space-x-1">
                {['check', 'heart', 'plus'].map((icon) => (
                    <Link key={icon} href={icon === 'plus' ? `/create-story?currId=${currId}` : '#'} className={`w-7 h-7 rounded-full flex items-center justify-center ${isDark ? 'bg-secondary hover:bg-secondary/80' : 'bg-accent hover:bg-accent/80'} transition-all duration-200 group shadow-md hover:shadow-lg cursor-pointer`}>
