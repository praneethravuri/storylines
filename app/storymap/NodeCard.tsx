...
const NodeCard: React.FC<NodeItems> = ({
  isDark,
  title,
  author,
  createdAt,
        ...
        {['check', 'heart', 'plus'].map((icon) => (
          <button
            ...
            className={`w-8 h-8 rounded-full flex items-center justify-center ${
              isDark ? 'bg-secondary hover:bg-secondary/80' : 'bg-accent hover:bg-accent/80'
            } transition-all duration-200 group shadow-md hover:shadow-lg cursor-pointer`}
          >
            {icon === 'check' && (
              <Check
